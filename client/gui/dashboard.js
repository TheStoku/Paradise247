...
class Dashboard {
    ...
    generateDashString() {
        let message = "";
        let joins = localClient.getData("joins");
        let money = localClient.getData("money");
        let bank = localClient.getData("bank");
        let kills = localClient.getData("kills");
        let deaths = localClient.getData("deaths");
        let suicides = localClient.getData("suicides");
        let kdRatio = killDeathRatio(kills, deaths).toFixed(2);
    ...
    generateAchivementsString() {
        ...
        Achievement.getFilteredList().forEach(element => {
            let groupCompletion = Achievement.getGroupCompletion(element, localClient.getData(element));
            let nextLevel = Achievement.getGroupNextLevelValueString(element, localClient.getData(element));
