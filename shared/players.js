...
let Players = Array.apply(null, Array(MAX_PLAYERS)).map(function () {});
...
class Player {
    ...
    setLanguage(param) {
        if (param != "undefined") {
            let language = localeFiles.findIndex( locale => locale == param.toString() )
    ...
    passwordLogin(password) {
        ...
        if (checkPasswordQuery(this.client.name, password)) { this.completeLogin(); }
        else {
    ...
    completeLogin() {
        ...
        if (loadPlayerDataQuery(this.client)) {
            ...
            let lastSeen = new Date(this.db.last_seen * 1000);
            let lastLogin = lastSeen.toLocaleDateString('pl-PL') + " " + lastSeen.toLocaleTimeString('pl-PL');
    ...
    increaseKills() {
        ...
        if (this.session.spree % 5 == 0) {
            let reward = earningBase.spree * this.session.spree;
    ...
    increaseMileage(inc) {
        let newMileage = ((Number(inc)/1000) + Number(this.db.mileage)).toFixed(2);
