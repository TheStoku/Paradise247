...
addEventHandler("onPedSpawn", (event, ped) => {
    if (ped && ped.type == ELEMENT_PLAYER && ped.isSyncer && localClient.getData("dojo") == null) {
        let spawnType = localClient.getData("spawnType");
        let isLoggedIn = localClient.getData("isLoggedIn");
            ...
            if (spawnType != null) {
                switch (spawnType) {
                    case 0:
                        if (!preInit) { spawnScreen.enter(); }
                            ...
                            setTimeout(() => {
                                ...
                                let camera = Spawn.get(spawnScreen.skinSelection).camera;
                                ...
                                if (isLoggedIn < 1 ) {
                                    ...
                                    if (isLoggedIn == -1) {
                                        ...
                                        let title = Locale.getString("client.gui.information");
                                        let popupMessage = Locale.getString("client.gui.notRegistered", localClient.name);
                                        let rulesMessage = Locale.getString("client.gui.rulesMessage");
                                        
                                        new Popup(title, popupMessage, null, null, function(){
                                            new Prompt(title, rulesMessage, null, null, null, function(){ new LoginWindow(isLoggedIn) }, function(){ triggerNetworkEvent("gui.disconnect"); });
                                        });
                    ...
                    case 3:
                        ...
                        let hospital = gameLocations[gta.game][id];
