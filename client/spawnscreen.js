...
let pricedown = null;
let myFont = lucasFont.createDefaultFont(14.0, "Roboto", "Light");
let myFont3 = lucasFont.createDefaultFont(14.0, "Tahoma", "Bold");
...
class SpawnScreen {
    ...
    static set isEnabled(value) { this.isEnabled = value; }

    static get skinSelection() { return this.skinSelection; }
    static set skinSelection(value) { this.skinSelection = value; }

    static get weaponSelection() { return this.weaponSelection; }
    static set weaponSelection(value) { this.weaponSelection = value; }

    right() { this.skinSelection < this.numberOfSpawns ? this.skinSelection++ : this.skinSelection = 0; this.weaponSelection = 0; this.update(true); }
    left() { this.skinSelection > 0 ? this.skinSelection-- : this.skinSelection = this.numberOfSpawns; this.weaponSelection = 0; this.update(true); }
    
    up() { this.weaponSelection < this.numberOfWeapons ? this.weaponSelection++ : this.weaponSelection = 0; this.update(false); }
    down() { this.weaponSelection > 0 ? this.weaponSelection-- : this.weaponSelection = this.numberOfWeapons; this.update(false); }
    ...
    update(fade) {
        if (typeof Spawn.get(this.skinSelection) != "undefined") {
            ...
            let skin = Spawn.get(this.skinSelection).skin;
    ...
    armSelectedWeapon() {
        ...
        let selectedWeaponId = Number(this.weapons[this.weaponSelection]);
    ...
    armDefaultWeapons() {
        ...
        let defaultWeapons = [...Spawn.get(this.skinSelection).weaponSet];
    ...
    spawn() {
        ...
        let spawn = Spawn.get(this.skinSelection);
        let teamLevel = spawn.team.level;
        let xp = Number(localClient.getData("xp"));
    ...
    toggle(showMessage) {
        ...
        if (showMessage) {
            let msg = this.isEnabledNextTime ? Locale.getString("SpawnscreenEnabled") : Locale.getString("SpawnscreenDisabled");
            message(msg);
        }
    }
}
...
addEventHandler("onKeyDown", (event, virtualKey, physicalKey, keyModifiers) => {
    if (spawnScreen.isEnabled && dashboard && dashboard.mainWindow && !dashboard.mainWindow.isShown()) {
        // Rotate skins
        if (virtualKey == SDLK_RIGHT) { spawnScreen.right(); }
        else if (virtualKey == SDLK_LEFT) { spawnScreen.left(); }
        // Rotate weapons
        else if (virtualKey == SDLK_UP) { spawnScreen.up(); }
        else if (virtualKey == SDLK_DOWN) { spawnScreen.down(); }
        // Spawn
        else if (virtualKey == SDLK_RETURN) { spawnScreen.spawn(); }
        else return;
    }
});
...
bindEventHandler("OnResourceReady", thisResource, function (event, resource) {
    ...
    let fontStream = openFile("files/pricedown.ttf");
...
addEventHandler("OnDrawnHUD", (event) => {
    ...
    if (spawnScreen && spawnScreen.isEnabled && dashboard && !dashboard.isShown && isConnected) {
        let spawn = Spawn.get(spawnScreen.skinSelection);
        //let teamColor = spawn.team.color; // GTAC font doesn't render colourcodes.
        let teamName = spawn.team.name;
        let teamLevel = spawn.team.level;
        let xp = Number(localClient.getData("xp"));
        let gangCarName = getVehicleNameFromModelId(spawn.team.car);
        let selectedWeaponName = weaponNames[gta.game][Number(spawnScreen.weapons[spawnScreen.weaponSelection])];
