	...
	if (localPlayer != null && isConnected) {
		if (entity == localPlayer) {
            ...
            if (entity.vehicle && gta.game == 1 && entity.vehicle.model == MODELVEHICLE_CAR_DODO && localPlayer.seat == 0) {
                let vehHandle = gta.STORE_CAR_PLAYER_IS_IN(gta.GET_PLAYER_ID());

                if (gta.IS_CAR_IN_AIR(vehHandle) && !gta.IS_CAR_IN_WATER(vehHandle) && dodoFlightStart == -1) {
                    dodoFlightStart = gta.tickCount;
                } else if ((!gta.IS_CAR_IN_AIR(vehHandle) || gta.IS_CAR_IN_WATER(vehHandle)) && dodoFlightStart != -1) {
                    finishFlight();
                }
            ...
            let dojoId = localClient.getData("dojo");

            if (dojoId != null) {
                let boundraries = null;
                let dojoCenter = null;
                let testPos = new Vec2(localPlayer.position.x, localPlayer.position.y);
                ...
                let polyTest = inPoly.apply(null, boundraries);
    
                //message(polyTest.toString());

                if (!polyTest) {
                    let heading = getHeadingFromPosToPos(dojoCenter, localPlayer.position);
                    let pos = getPosBehindPos(localPlayer.position, heading, 1.0);
...
function finishFlight() {
    let flightTime = Math.round((gta.tickCount - dodoFlightStart)/1000);
