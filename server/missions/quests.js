...
class Quest {
    ...
    static check(client, task, target = null, type = null) {
        ...
        let quest = Quests[player.db.quests];
        ...
        if (quest.task == task) {
            ...
            if (quest.repeats > player.session.questRepeats) {
                ...
                let reward = earn(client, quest.reward * quest.repeats, true);
    ...
    static print(client) {
        ...
        let task = Quests[player.db.quests].task;
        let repeats = Quests[player.db.quests].repeats;
        let target = Quests[player.db.quests].target;
        let type = Quests[player.db.quests].type;
