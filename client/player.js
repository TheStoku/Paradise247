...
addNetworkHandler("setPlayerAnimation", (pedId, animGroup, animId, lockControlsTime) => {
    let ped = getElementFromId(Number(pedId));
...
addNetworkHandler("setPlayerHeading", (client,heading) => {
    let ped = getElementFromId(client);
...
addCommandHandler("pos", (command, params, client) => {
    let x = parseFloat(localPlayer.position.x).toFixed(2);
    let y = parseFloat(localPlayer.position.y).toFixed(2);
    let z = parseFloat(localPlayer.position.z).toFixed(2);
    let heading = parseFloat(localPlayer.heading).toFixed(2);
    let message = params.toString() ? `${params} ${x.toString()}, ${y.toString()}, ${z.toString()} ${heading}` : `${x.toString()}, ${y.toString()}, ${z.toString()} ${heading}`;
