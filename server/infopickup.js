...
class InfoPickup {
	...
	static get(pickup) {
		const index = InfoPickups.findIndex( (infoPickup) => infoPickup.pickup.id == pickup.id );

		return index > -1 ? InfoPickups[index] : null;
	}
	...
	enter(client) {
		...
		const collectedPickups = Player.get(client).db.infoPickups;
