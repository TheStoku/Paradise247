...
const BOUNDARIES_RAILWAYS = [new Vec2(1285.87, -40.24), new Vec2(1310.47, -30.99), new Vec2(1318.27, -23.06), new Vec2(1344.06, -23.80), new Vec2(1343.60, -136.19), new Vec2(1337.27, -142.26), new Vec2(1284.56, -142.23)];
const BOUNDARIES_SSV = [new Vec2(-1151.94, 125.25), new Vec2(-1151.86, 108.84), new Vec2(-1166.64, 98.83), new Vec2(-1176.82, 83.51), new Vec2(-1180.51, 65.01), new Vec2(-1180.46, 23.06), new Vec2(-1187.87, 23.06), new Vec2(-1187.87, 84.11), new Vec2(-1218.27, 83.88), new Vec2(-1232.04, 70.11), new Vec2(-1248.46, 53.36), new Vec2(-1248.46, 23.74), new Vec2(-1254.26, 23.73), new Vec2(-1255.11, 85.88), new Vec2(-1255.75, 161.44), new Vec2(-1242.54, 161.48), new Vec2(-1242.54, 125.98)];
const BOUNDARIES_STADIUM = [new Vec2(-92.19, -230.32), new Vec2(-139.38, 2.30), new Vec2(-43.61, 72.18), new Vec2(55.13, 57.37), new Vec2(94.03, -59.94), new Vec2(84.24, -211.99)];

const CENTER_RAILWAYS = new Vec3(1317.10, -78.56, 16.35);
const CENTER_SSV = new Vec3(-1215.05, 101.64, 68.63);
const CENTER_STADIUM = new Vec3(-16.04, -80.21, 19.11);

const localTestPos = new Vec2(0.0, 0.0);
...
addEventHandler('OnEntityProcess', function(event, entity) {
	...
	if (localPlayer !== null && isConnected) {
		if (entity === localPlayer) {
			// Dodo flight time.
			const lVehicle = localPlayer.vehicle;

			if (lVehicle && gta.game === GAME_GTA_III && lVehicle.model === MODELVEHICLE_CAR_DODO && localPlayer.seat === 0) {
			...
			if (dojoId != null) {
				...
				localTestPos.x = localPlayer.position.x;
				localTestPos.y = localPlayer.position.y;

				if (dojoId <= 2) { 
					boundraries = BOUNDARIES_RAILWAYS;
					dojoCenter = CENTER_RAILWAYS;
				} else if (dojoId === 3) { 
					boundraries = BOUNDARIES_SSV;
					dojoCenter = CENTER_SSV;
				} else if (dojoId === 4) { 
					boundraries = BOUNDARIES_STADIUM;
					dojoCenter = CENTER_STADIUM;
				}

				if (boundraries) {
					const polyTest = inPoly(localTestPos, ...boundraries);

					if (!polyTest) {
						const heading = getHeadingFromPosToPos(dojoCenter, localPlayer.position);
						localPlayer.position = getPosBehindPos(localPlayer.position, heading, 1.0);
					}
				}
			}
		}
