...
class Vehicle {
	...
	init() {
		...
		if (this.db.colour1 == -1 && this.db.colour2 == -1) {
			this.instance.randomiseColours();
		}
	...
	static spawnTempVehicle(client, params) {
		...
		const modelId = getVehicleModelIdFromParams(params, gta.game);
		const vehicle = gta.createVehicle(modelId, client.player.position);
...
addNetworkHandler('onDodoFlightFinish', function(client, flightTime) {
	...
	// const current = new Date(flightTime * 1000).toISOString().slice(11, 19);
	// const overall = new Date(player.db.dodoFlightTime * 1000).toISOString().slice(11, 19);

	triggerNetworkEvent('smallMessage', client, player.getLocale().getString('flightTime', flightTime.toString()), 2000, 2);
});
