	...
	if (localPlayer != null && isConnected) {
        if (entity == localPlayer) {
            if (entity.vehicle != null) {
                ...
                // Count speed
                let speed = (getSpeedFromVelocity(velocity) * 100);
                ...
                if (speedo.mileage == -1) {
                    let mileage = vehicle.getData("mileage");
                ...
                if (gta.tickCount - speedo.tick >= 250 && localPlayer.seat == 0) {
                    let inc = Math.round((getSpeedFromVelocity(velocity) * 100)/4);
                    speedo.mileage += inc;
                    speedo.tick = gta.tickCount;

                    // Store mileage to vehicle data
                    let formattedMileage = (speedo.mileage/1000).toFixed(2);
                    let formattedInc = (inc/1000).toFixed(2);
...
addEventHandler("OnDrawnHUD", (event) => {
    ...
    if (speedo.speed != -1) {
        let units = speedo.metric ? "KM/h" : "MPh";
